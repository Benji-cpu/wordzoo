...
export async function exportUserData(userId: string): Promise<UserDataExport> {
  ...
  const [
    ...
    sql`SELECT word_id, status, ease_factor, interval_days, learning_step, lapses, next_review_at, times_reviewed, times_correct, last_reviewed_at, direction FROM user_words WHERE user_id = ${userId}`,
    sql`SELECT phrase_id, status, ease_factor, interval_days, learning_step, lapses, next_review_at, times_reviewed, times_correct, last_reviewed_at FROM user_phrases WHERE user_id = ${userId}`,
