...
/**
 * Seeds dialogue scenes for Indonesian. Can be called standalone or from seed.ts.
 * @param sql - Neon query function. If not provided, creates one from DATABASE_URL.
 * @param onlyScenes - Optional array of 1-based scene numbers to seed (e.g. [1, 3]).
 */
export async function seedDialogueScenes(sql: NeonQueryFunction<false, false>, onlyScenes?: number[] | null) {
  // 1. Delete legacy Indonesian scenes (d1000000-...-000000000001 through 003)
  console.log('Deleting legacy Indonesian scenes...');
  await sql`
...
// Allow standalone execution: `npx tsx lib/db/seed-dialogues.ts`
const isDirectRun = process.argv[1]?.endsWith('seed-dialogues.ts');
if (isDirectRun) {
  (async () => {
    ...
    try {
      await seedDialogueScenes(sql, onlyScenes);
    } catch (error) {
