...
import { getPathProgress } from '@/lib/services/path-service';
...
export async function GET(
  ...
  try {
    const progress = await getPathProgress(session.user.id, parsed.data.pathId);
    return NextResponse.json<ApiResponse<typeof progress>>({
      data: progress,
      error: null,
    });
  } catch (error) {
