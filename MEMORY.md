...
- **Gemini TTS is capped at 100 requests/day/model on the free tier**, and it is the *only* voice that works — Cloud TTS has never had billing enabled, so every call 403s and falls through to `gemini-2.5-flash-tts`. The cap is shared across words, phrases, narrations and dialogues. `seed-audio.ts` now aborts on the first daily-quota 429 (it used to make 175 doomed calls after the cap); re-run to resume, it skips what already has audio. CRM #358. The IMAGE model is a separate bucket — 163 images generated the same day with no throttling.
