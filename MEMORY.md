...
- **Gemini TTS is capped at 100 requests/day/model on the free tier**, and it *was* the only voice that worked — ~~Cloud TTS has never had billing enabled~~ **billing was REOPENED 2026-09-17 on project `473497770902`, and that project is on `mystechcards@gmail.com`** (b.hemsonstruthers 403s on it — wrong account, not a missing grant; MysTech's MEMORY.md says the good voice is live in both apps). Re-test before quoting the Gemini fallback as the only voice. The cap is shared across words, phrases, narrations and dialogues. `seed-audio.ts` now aborts on the first daily-quota 429 (it used to make 175 doomed calls after the cap); re-run to resume, it skips what already has audio. CRM #358. The IMAGE model is a separate bucket — 163 images generated the same day with no throttling.
